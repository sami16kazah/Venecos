...
export default function SupportServicePage() {
  const [formData, setFormData] = useState({
    title: { ar: 'خدمة الدعم الفني والصيانة', en: 'Technical Support & Maintenance', fr: 'Support technique et maintenance', de: 'Technischer Support & Wartung' },
    priceFrom: 20,
    priceTo: 200,
    daysFrom: 1,
    daysTo: 3,
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between bg-venecos-black/90 p-6 rounded-2xl border border-venecos-gold/20 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-green-600 flex items-center justify-center text-white text-2xl shadow-lg">
            <MdHeadset />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">إدارة خدمة الدعم الفني (Tech Support)</h1>
            <p className="text-xs text-white/60">باقات ساعات صيانة وحل مشكلات السيرفرات والمواقع</p>
          </div>
        </div>
      ...
      <div className="bg-venecos-black/70 border border-white/10 rounded-2xl p-6 space-y-4">
        <h3 className="text-sm font-bold text-venecos-gold">أسعار ومدة تنفيذ الدعم الفني</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-xs font-bold text-white/80 mb-1">المدة من</label>
          ...
          <div>
            <label className="block text-xs font-bold text-white/80 mb-1">المدة إلى</label>
